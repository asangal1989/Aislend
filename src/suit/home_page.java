...
public class home_page extends global_variables{
	...
	@Test	
	  public void homepage() {
		...
		try
		{
			...
			prop.load(in);
			log_system.info("*************************Prepare Data Set for "+this.getClass().getSimpleName()+ "from excel "+prop.getProperty("aislend_HomePage_FileName")+" *******************************************************************");
			test_data=getExcelSheet.readInputs(path_lib_testcase+prop.getProperty("aislend_HomePage_FileName"), prop.getProperty("aislend_HomePage_SheetName"));
			log_system.info("*************************** Data set Ready for "+this.getClass().getSimpleName()+" ***************************************");
			log_system.info("******************************* Intializing Recorder *************************************");
			/*recorder.startRecording(path_lib_record,prop.getProperty("aislend_HomePage_SheetName"));*/
			path_lib_recoderfull=path_lib_record+prop.getProperty("aislend_HomePage_SheetName")+".mov";
			...
			for(List TestData:test_data)
			  {
				  ...
				  output.writeResult(UseCase_ID,TestCase_ID,path_lib_testcase+prop.getProperty("aislend_HomePage_FileName"), prop.getProperty("aislend_HomePage_SheetName"),result_log,test_data.indexOf(TestData),Description,Expected_result,this.getClass().getName());				  
