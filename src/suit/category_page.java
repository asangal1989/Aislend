...
public class category_page extends global_variables{
	...
	@Test	
	  public void CategoryPage() {
		...
		try
		{
			...
			prop.load(in);
			log_system.info("*************************Prepare Data Set for "+this.getClass().getSimpleName()+ "from excel "+prop.getProperty("aislend_CategoryPage_FileName")+" *******************************************************************");
			...
			for(List TestData:test_data)
			  {
				  ...
				  output.writeResult(UseCase_ID,TestCase_ID,path_lib_testcase+prop.getProperty("aislend_CategoryPage_FileName"), prop.getProperty("aislend_CategoryPage_SheetName"),result_log,test_data.indexOf(TestData),Description,Expected_result,this.getClass().getName());				  
